...
do
    ...
    mysqldump --password=${OPENSHIFT_MYSQL_DB_PASSWORD} -u ${OPENSHIFT_MYSQL_DB_USERNAME} -h ${OPENSHIFT_MYSQL_DB_HOST} -P ${OPENSHIFT_MYSQL_DB_PORT} $db | gzip > ${OPENSHIFT_DATA_DIR}/${BACKUP_NAME}.sql.gz
done
